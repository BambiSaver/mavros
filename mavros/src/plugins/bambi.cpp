...
public:
  ...
  void initialize(UAS &uas_)
  {
    ...
    bambi_nh.param<std::string>("frame_id", frame_id, "map");
    // Important tf subsection
    // Report the transform from world to base_link here.
    bambi_nh.param("tf/send", tf_send, false);
    bambi_nh.param<std::string>("tf/frame_id", tf_frame_id, "map");
    bambi_nh.param<std::string>("tf/child_frame_id", tf_child_frame_id, "base_link");
    // Debug tf info
    // broadcast the following transform: (can be expanded to more if desired)
    // NED -> aircraft
    bambi_nh.param("tf/send_fcu", tf_send_fcu, false);

    local_position = bambi_nh.advertise<geometry_msgs::PoseStamped>("pose", 10);
    local_velocity = bambi_nh.advertise<geometry_msgs::TwistStamped>("velocity", 10);
    local_odom = bambi_nh.advertise<nav_msgs::Odometry>("odom",10);
  }
